...
import { DEFAULT_UPLOAD_POLICY, type KnowledgeBase } from '@/lib/knowledge-helpers'
...
it('shows version identifiers and embedding selection with read-only role confirmation, then requires reconfirmation after drift', async () => {
  const submit = vi.fn().mockRejectedValueOnce(new Error('Default configuration changed'))
  ...
  expect(within(dialog).getByRole('combobox', { name: 'Embedding model' })).toBeInTheDocument()
  ...
  expect(within(dialog).getByText('Default configuration changed')).toBeInTheDocument()
...
it('refreshes all four default roles on return from settings and creates without overrides', async () => {
  ...
  expect(screen.queryByText('Customize indexing models')).not.toBeInTheDocument()
...
it.each(['header', 'documents'])(
  ...
  async entry => {
    const retry = vi.fn()
    const reindex = vi.fn().mockRejectedValueOnce(new Error('Default configuration changed'))
    render(
      <KnowledgeBaseDetail
        kb={{ ...kb, status: 'error' }}
        uploadPolicy={DEFAULT_UPLOAD_POLICY}
